...
def change_password(request):
    redirect_to = request.GET.get('from', reverse('home'))
    if request.method == 'POST':
        form = ChangePasswordForm(request, user=request.user)
        if form.is_valid():
            user = request.user
            new_pass_word = form.cleaned_data['new_password']
            user.set_password(new_pass_word)
            user.save()
            auth.logout(request)
